...
export default function App() {
  ...
  // Onboarding Form Details (Auto-generated Session ID)
  ...
  // Transition Step State (Jarvis steps)
  ...
  const [systemStatus, setSystemStatus] = useState('ready'); // 'loading' | 'ready' | 'error'
  ...
  useEffect(() => {
    ...
    if (transitionStep < 3) {
      ...
      const timer = setTimeout(() => {
        setScreen('chat');
        simulateTyping(tempReply, tempIsFollowUp);
      }, 700);
      return () => clearTimeout(timer);
    }
  ...
  return (
    <div className="min-h-screen relative flex flex-col justify-between overflow-x-hidden font-sans">
      ...
      <main className="flex-1 flex flex-col items-center justify-center py-8 z-10 w-full">
        <AnimatePresence mode="wait">
          ...
          {screen === 'transition' && (
            ...
            >
              {/* Blur gradient background in activation */}
              ...
              <div className="flex flex-col items-center max-w-md w-full text-center relative z-10">
                ...
                {/* Pulsing Active Boot Step Text */}
                <motion.h2 
                  key={transitionStep}
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -10 }}
                  className="text-2xl font-bold font-display text-zinc-100 tracking-tight mb-4 min-h-[36px]"
                >
                  {getTransitionString(transitionStep)}
                </motion.h2>
                
                <p className="text-zinc-550 text-xs font-mono mb-8 uppercase tracking-widest animate-pulse">
                  System Calibration In Progress
                </p>

                {/* Smooth growing progress line */}
                <div className="w-64 h-1 bg-zinc-950 border border-zinc-900 rounded-full overflow-hidden mb-8 relative">
                  <motion.div 
                    initial={{ width: "0%" }}
                    animate={{ width: `${fakeProgressPercent}%` }}
                ...
                {/* Staggered Boot Progress checklist */}
                <div className="w-56 space-y-3.5 text-left font-mono text-xs select-none">
                  <div className="flex items-center gap-3">
                    ...
                    <span className={`font-semibold transition-colors ${transitionStep >= 0 ? 'text-zinc-400 font-bold' : 'text-zinc-700'}`}>
                      Profile Loaded
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className={`font-semibold transition-colors ${transitionStep >= 1 ? 'text-zinc-400 font-bold' : 'text-zinc-700'}`}>
                      Question Engine Configured
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className={`font-semibold transition-colors ${transitionStep >= 2 ? 'text-zinc-400 font-bold' : 'text-zinc-700'}`}>
                      Difficulty Vectors Locked
                    </span>
                  </div>
                </div>
          ...
          {screen === 'chat' && (
            ...
            >
              <div className="flex-1 flex flex-col justify-between h-full overflow-hidden">
                <Chat messages={messages} isThinking={isThinking} sessionStatus={sessionStatus} isFocusMode={isFocusMode} setIsFocusMode={setIsFocusMode} />
                <div className="w-full max-w-[800px] mx-auto px-4 mt-2">
                  <InputBox onSendMessage={handleSendMessage} disabled={isThinking} onStartTyping={() => setIsFocusMode(false)} />
                </div>
              </div>
              <div className="w-[320px] shrink-0 hidden lg:block border-l border-zinc-900 pl-8 h-full overflow-hidden">
                <LiveFeedbackPanel evaluation={evaluation} sessionStatus={sessionStatus} />
              </div>
            </motion.div>
          )}
      ...
      {/* Floating Difficulty Scaling Toast Notification */}
      <AnimatePresence>
        {showDifficultyToast && (
      ...
      {/* Floating Basics Toast Notification */}
      <AnimatePresence>
        {showBasicsToast && (
